...
> 1. 在项目中新建一个文件夹``source``用于存放jdk源码

> 2. 将jdk安装路径中``src.zip``压缩包，解压到``source``文件夹中

> 3. 设置debugger 不跳入jdk classes问题

![image-20211115091625624](https://chenfuyuan-markdown-img.oss-cn-shenzhen.aliyuncs.com/20211115091627.png)

在 Settings > Build、Execution、Deployment > Debugger > Stepping 中，将Do not step into the classes 打钩去掉



> 4. 配置调试时读取jdk 变为 读取``source``文件夹

![image-20211115091951597](https://chenfuyuan-markdown-img.oss-cn-shenzhen.aliyuncs.com/20211115091952.png)

在 Project Struct > SDKs 中 将 ``jdk\jdk1.8\src.zip``删除，并添加``source``文件夹路径
...
![image-20211115092308132](https://chenfuyuan-markdown-img.oss-cn-shenzhen.aliyuncs.com/20211115092309.png)

点击**File–>Settings–>Build,Execution,Deployment–>Compiler**，将**Build process heap size(Mbytes)**选项加大，默认值是700



> 2. 程序包com.sun.tools.javac.*不存在

![image-20211115092524278](https://chenfuyuan-markdown-img.oss-cn-shenzhen.aliyuncs.com/20211115092525.png)

点击**File–>Project Structure–>Libraries**，添加一个Java的Project Libraries

![image-20211115092649238](https://chenfuyuan-markdown-img.oss-cn-shenzhen.aliyuncs.com/20211115092650.png)
...
在``source``文件夹下，新建``src\awt``目录，新建``UNIXToolkit.java``文件
...
在``source``文件夹下，新建``src\font``目录，新建``FontConfigManager.java``文件
